...
function SpacesPanel({ spaces, onChanged, onError }: { spaces: Space[]; onChanged: () => Promise<void>; onError: (message: string) => void }) {
  ...
  return <><h1>Espaces documentaires</h1>
    ...
    <form className="admin-form inline-form" onSubmit={async (event) => {
      event.preventDefault();
      await api('/api/admin/categories', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify(category) }).then(async () => { setCategory({ spaceId: '', slug: '', nameFr: '', nameEn: '' }); await onChanged(); }).catch((error) => onError(error.message));
    }}><h2>Créer une catégorie</h2><select required value={category.spaceId} onChange={(event) => setCategory({ ...category, spaceId: event.target.value })}><option value="">Espace…</option>{spaces.map((space) => <option key={space.id} value={space.id}>{space.nameFr}</option>)}</select><input required placeholder="slug" value={category.slug} onChange={(event) => setCategory({ ...category, slug: event.target.value })}/><input required placeholder="Nom français" value={category.nameFr} onChange={(event) => setCategory({ ...category, nameFr: event.target.value })}/><input required placeholder="Nom anglais" value={category.nameEn} onChange={(event) => setCategory({ ...category, nameEn: event.target.value })}/><button className="primary">Créer</button></form>
    <div className="card-grid">{spaces.map((space) => <article className="admin-card" key={space.id}><h2>{space.nameFr}</h2><small>{space.slug} · {space._count?.documents || 0} documents · {space._count?.accessRules || 0} règles</small><ul>{space.categories?.map((item) => <li key={item.id}>{item.nameFr} / {item.nameEn}</li>)}</ul><button className="danger" onClick={async () => { if (window.confirm(`Archiver ${space.nameFr} ?`)) await api(`/api/admin/spaces/${space.id}`, { method: 'DELETE' }).then(onChanged).catch((error) => onError(error.message)); }}>Archiver</button></article>)}</div>
  </>;
}
