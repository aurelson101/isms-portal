...
type ConnectionWithCa = DirectoryConnection & { caCertificate: TrustedCaCertificate | null };
...
const validateFilter = (filter: string) => {
  if (!filter.startsWith('(') || !filter.endsWith(')') || filter.includes('\0')) {
...
export class DirectoryService {
  ...
  private client(connection: ConnectionWithCa, host: string) {
    ...
    if (secure && !connection.caCertificate) throw new Error('A trusted CA certificate is required for LDAPS');
    return new Client({
  ...
  private async bindWithFallback(connection: ConnectionWithCa) {
    const hosts = [connection.primaryHost, connection.secondaryHost].filter(Boolean) as string[];
    const errors: string[] = [];
    for (const host of hosts) {
      for (let attempt = 0; attempt <= connection.retries; attempt += 1) {
        const client = this.client(connection, host);
        try {
          await client.bind(connection.bindDn, this.crypto.decrypt(connection.encryptedBindSecret));
          return { client, host };
        } catch (error) {
          errors.push(`${host} attempt ${attempt + 1}: ${error instanceof Error ? error.message : 'unknown error'}`);
  ...
  async test(id: string) {
    ...
    if (!connection) throw new BadRequestException('Directory connection not found');
    ...
    try {
      ...
      results.tcp = await new Promise((resolve, reject) => {
        const socket = createConnection({ host: connection.primaryHost, port: connection.port });
      ...
      if (connection.protocol === 'LDAPS') {
        if (!connection.caCertificate) throw new Error('A trusted CA certificate is required for LDAPS');
        results.tls = await new Promise((resolve, reject) => {
          const socket = tlsConnect({
            ...
            socket.destroy();
            resolve({ ok: true, subject: peer.subject?.CN, validTo: peer.valid_to });
          });
          socket.setTimeout(connection.timeoutMs);
      ...
      try {
        const userResult = await client.search(connection.userBaseDn || connection.baseDn, {
          scope: 'sub', filter: connection.userFilter, sizeLimit: 1,
          attributes: [connection.usernameAttribute, connection.emailAttribute],
        });
        const groupResult = await client.search(connection.groupBaseDn || connection.baseDn, {
          scope: 'sub', filter: connection.groupFilter, sizeLimit: 1,
          attributes: [connection.groupAttribute, 'member'],
        });
        results.userSearch = { ok: true, count: userResult.searchEntries.length };
        results.groupSearch = { ok: true, count: groupResult.searchEntries.length };
      } finally {
        await client.unbind().catch(() => undefined);
      }
      await this.prisma.directoryConnection.update({
        where: { id }, data: { lastTestAt: new Date(), lastTestStatus: 'SUCCESS' },
      });
      ...
      await this.prisma.directoryConnection.update({
        where: { id }, data: { lastTestAt: new Date(), lastTestStatus: 'ERROR' },
      });
      return { status: 'ERROR', durationMs: Date.now() - startedAt, results, error: message };
    }
  }
  ...
  async synchronize(id: string) {
    ...
    try {
      const { client, host } = await this.bindWithFallback(connection);
      try {
        const result = await client.search(connection.groupBaseDn || connection.baseDn, {
          scope: 'sub',
        ...
        await this.prisma.directorySyncJob.update({
          where: { id: job.id }, data: { status: 'SUCCESS', details, finishedAt: new Date() },
        });
      ...
      const details = { error: error instanceof Error ? error.message : 'Unknown directory error' };
      await this.prisma.directorySyncJob.update({
        where: { id: job.id }, data: { status: 'ERROR', details, finishedAt: new Date() },
      });
