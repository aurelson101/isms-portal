    ...
    return new Promise((resolve, reject) => {
      ...
      socket.on('connect', () => {
        ...
        for (let offset = 0; offset < content.length; offset += 65536) {
          const chunk = content.subarray(offset, Math.min(offset + 65536, content.length));
      ...
      socket.on('end', () => {
        ...
        if (infected) return resolve({ status: 'INFECTED', signature: infected[1] });
        resolve({ status: 'ERROR', signature: response || 'Unknown ClamAV response' });
      });
      socket.on('timeout', () => {
        socket.destroy();
        reject(new ServiceUnavailableException('ClamAV scan timed out'));
      });
      socket.on('error', (error) => reject(new ServiceUnavailableException(`ClamAV unavailable: ${error.message}`)));
    });
  }
}
