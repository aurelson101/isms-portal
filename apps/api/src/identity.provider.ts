...
const cidrContains = (cidr: string, address: string) => {
  ...
  if (networkNumber === null || addressNumber === null || prefix < 0 || prefix > 32) return false;
...
export class TrustedProxyIdentityProvider implements IdentityProvider {
  ...
  resolve(request: IsmsRequest) {
    ...
    if (!username.trim() || username.length > 255 || groups.length > 8192) throw new UnauthorizedException();
    return {
