...
import { DemoIdentityProvider, TrustedProxyIdentityProvider } from './identity.provider';

@Injectable()
export class IdentityMiddleware implements NestMiddleware {
  private readonly providers = [new DemoIdentityProvider(), new TrustedProxyIdentityProvider()];

  use(req: IsmsRequest, res: Response, next: NextFunction) {
    const suppliedRequestId = req.headers['x-request-id'];
    req.correlationId = typeof suppliedRequestId === 'string' && /^[a-zA-Z0-9._-]{1,128}$/.test(suppliedRequestId)
      ? suppliedRequestId
      : randomUUID();
    ...
    const provider = this.providers.find((candidate) => candidate.supports(req));
