...
describe('AuthorizationService', () => {
  ...
  it('grants every permission to an administrator without a per-space rule', async () => {
    findMany.mockResolvedValue([{ id: 'space-it', slug: 'it', accessRules: [] }]);
    await expect(service.can(['ISMS-ADMINS'], 'space-it', 'administer')).resolves.toBe(true);
    await expect(service.permittedSpaces(['ISMS-ADMINS'], 'upload')).resolves.toHaveLength(1);
    expect(count).not.toHaveBeenCalled();
  });

  it('denies by default when no group rule grants the permission', async () => {
    count.mockResolvedValue(0);
    await expect(service.can(['Domain Users'], 'space-it', 'download')).resolves.toBe(false);
  });
  ...
  it('queries only active matching groups for standard users', async () => {
    ...
    expect(findMany).toHaveBeenCalledWith(expect.objectContaining({
      where: expect.objectContaining({
        accessRules: { some: { group: { name: { in: ['ITAD'] }, active: true }, search: true } },
      }),
    }));
  });
});
