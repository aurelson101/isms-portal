...
const worker = new Worker<{ connectionId: string }>(
  'directory-sync',
  async (job) => {
    log('directory-sync.started', { jobId: job.id, connectionId: job.data.connectionId });
    ...
    if (result.status !== 'SUCCESS') throw new Error('Directory synchronization failed');
    return result;
  },
  { connection, concurrency: Number(process.env.DIRECTORY_SYNC_CONCURRENCY || 2) },
);
...
async function schedule() {
  const now = Date.now();
  const connections = await prisma.directoryConnection.findMany({ where: { enabled: true } });
  for (const directoryConnection of connections) {
    ...
    const due = !last || now - last.startedAt.getTime() >= directoryConnection.syncIntervalMinutes * 60000;
    if (due) {
      const intervalBucket = Math.floor(now / (directoryConnection.syncIntervalMinutes * 60000));
      await queue.add(
        'synchronize',
