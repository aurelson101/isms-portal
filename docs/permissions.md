...
Un compte administrateur enregistré possède toutes les permissions par défaut,
même sans règle par espace. L’accès à `/admin` est en plus contrôlé au proxy et
par le garde NestJS.

Le droit global d'administration n'est jamais attribué par une règle de groupe.
Il est réservé aux comptes administrateurs créés ou ajoutés dans l'interface
d'administration.
